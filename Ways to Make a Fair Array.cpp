...
int waysToMakeFair(vector<int>& nums) {
        int n=nums.size(),a[n+2],ekn[n+2],pic[n+2],ekn1[n+2],pic1[n+2];
        for(int i=0;i<=n+1;i++)ekn[i]=pic[i]=ekn1[i]=pic1[i]=a[i]=0;
        int sum1=0,sum2=0;
        for(int i=1;i<=n;i++)
        {
            a[i]=nums[i-1];
            if(i&1)
            {
                ...
                sum1+=nums[i-1];ekn[i]=sum1;
            }
            else
            {
                sum2+=nums[i-1]; ekn[i]=sum2;
            }
        }sum1=sum2=0;
        for(int i=n;i>0;i--)
        {
            if(i&1)
            {
                sum1+=nums[i-1];pic[i]=sum1;
            }
            else
            {
                sum2+=nums[i-1];pic[i]=sum2;
            }
        }int cr_h=0;
        for(int i=1;i<=n;i++)
        {
            int ek=pic[i]+ekn[i-1];
            int dui=ekn[i]+pic[i+1];
            if(ek==dui)cr_h++;
        }
        return cr_h;
    }
