...
void print(priority_queue< T >v) {
    while(!v.empty())cout<<v.top()<<whp,v.pop();
    cout << ses;
}
...
void insert(string&  str, v99 len)
{
    node* curr = root;
    for (v99 i = 0; i < len; i++) {
        v99 id = str[i] - 'a';
...
void berkori(node *curr,string s)
{
    if(curr->visi==true)return;
    curr->visi=true;
    for(v99 i=0;i<26;i++)
    {
        if(curr->next[i]!=NULL)
        {
            string p=s+char(i+'a');
            berkori(curr->next[i],p);
        }
    }
    if(curr->endmark==true)
    {
        pq.push(s);print(s,"sadf");
        while(pq.size()>3)
                pq.pop();
    }
}
vector< vector<string> > ans;
void ok(priority_queue<string>p,v99 ind)
{
    //vector<string>a;
    ans[ind].clear();
    while(!p.empty())ans[ind].pb(p.top()),p.pop();
    reverse(sob(ans[ind]));
}
void alabu(node* curr,v99 ind,string s)
{
    v99 n=Sea.size();
    if(ind==n)
    {
        berkori(curr,s);
        if(ind!=0)ok(pq,ind-1);
        return;
    }
    v99 val=Sea[ind]-'a';
    if(curr->next[val]!=NULL and ind<n)alabu(curr->next[val],ind+1,s+Sea[ind]);
    berkori(curr,s);
    if(ind!=0)ok(pq,ind-1);
}
...
void solve()
{
    ...
    for(v99 i=0;i<n;i++)read(products[i]),insert(products[i],products[i].size());
    read(Sea);
    v99 m=Sea.size();
    ans.resize(m);
    alabu(root,0LL,"");
    for(auto i:ans)if(i.size())print(i);
}
